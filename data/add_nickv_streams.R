...
#
# Notes:
#  1. After completed sent shapefile data to Lea from:
#     a, output_chehalis_streams.R, b, output_chehalis_reach_points.R
#
...
wb = tibble(waterbody_id = remisc::get_uuid(1L),
            waterbody_name = "Unnamed trib to Lucas Cr",
            waterbody_display_name = "Unnamed trib to Lucas Cr",
            latitude_longitude_id = ll_id,
            stream_catalog_code = "23",
            tributary_to_name = "Lucas Creek",
            obsolete_flag = 0L,
...
# Get info from cat_llid
st_cat = cat_llid_chehalis %>%
  filter(llid == ll_id | stringi::stri_detect_fixed(cat_name, "0923", max_count = 1))
...
# Check if the waterbody_id already exists in stream table
wb_id_2 = streams_st %>%
  filter(stringi::stri_detect_fixed(waterbody_display_name, "0923", max_count = 1))

# Check if the waterbody_id already exists in stream table
wb_id_3 = streams_st %>%
  filter(stringi::stri_detect_fixed(cat_code, "23.0923", max_count = 1))
...
#======== Unnamed trib to Bernier Creek One =============================
...
# Create new entry in waterbody_lut
wb = tibble(waterbody_id = remisc::get_uuid(1L),
            waterbody_name = "Unnamed trib to Bernier Cr",
            waterbody_display_name = "Unnamed trib to Bernier Cr",
            latitude_longitude_id = ll_id,
            stream_catalog_code = "23",
            tributary_to_name = "Bernier Creek",
            obsolete_flag = 0L,
            obsolete_datetime = as.POSIXct(NA))
...
#======== Unnamed trib to Bernier Creek Two =============================
...
#======== Unnamed trib to Bernier Creek Three =============================
...
#======== Unnamed trib to Beaver Creek One =============================
...
# Create new entry in waterbody_lut
wb = tibble(waterbody_id = remisc::get_uuid(1L),
            waterbody_name = "Unnamed trib to Beaver Cr",
            waterbody_display_name = "Unnamed trib to Beaver Cr",
            latitude_longitude_id = ll_id,
            stream_catalog_code = "23",
            tributary_to_name = "Beaver Creek",
            obsolete_flag = 0L,
            obsolete_datetime = as.POSIXct(NA))
...
#======== Unnamed trib to Beaver Creek Two =============================
...
# Get info from cat_llid
st_cat = cat_llid_chehalis %>%
  filter(llid == ll_id | stringi::stri_detect_fixed(cat_name, "1309", max_count = 1))

# Check if the waterbody_id already exists in stream table
wb_id = streams_st %>%
  filter(llid == ll_id)

# Check if the waterbody_id already exists in stream table....Result....Florence Creek already exists
wb_id_2 = streams_st %>%
  filter(stringi::stri_detect_fixed(waterbody_display_name, "1309", max_count = 1))
...
# Pull out wb_id...already exists so use existing wb_id
# new_wb_id = wb$waterbody_id
...
# Update trib to name on prod ==================================================
qry = glue::glue("UPDATE spawning_ground.waterbody_lut ",
                 "SET tributary_to_name = 'NF Johns River' ",
