...
wb = tibble(waterbody_id = remisc::get_uuid(1L),
            waterbody_name = "Unnamed trib to WF Wishkah R",
            waterbody_display_name = "Unnamed trib to WF Wishkah R",
            latitude_longitude_id = ll_id,
            stream_catalog_code = "22",
            tributary_to_name = "WF Wishkah River",
            obsolete_flag = 0L,
...
# Check if the waterbody_id already exists in stream table
wb_id_2 = streams_st %>%
  filter(stringi::stri_detect_fixed(waterbody_display_name, "0508", max_count = 1))

# Check if the waterbody_id already exists in stream table
wb_id_3 = streams_st %>%
  filter(stringi::stri_detect_fixed(cat_code, "22.0508", max_count = 1))
...
qry = glue("UPDATE waterbody_lut ",
           ...
           "tributary_to_name = 'MF Wildcat Creek' ",
