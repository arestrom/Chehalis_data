...
#  9. Only need WRIA 22 for Kim and Curt if species == Chum.
#
# AS 2020-07-14
#================================================================
...
#============================================================================
# Filter surveys to those where counts for chum were intended
#============================================================================
...
#============================================================================
# Filter surveys to those where counts for "fall" chum were intended
#============================================================================
...
header_data_adj = header_data %>%
  ...
  mutate(run = if_else(run == "Unknown",
                       "Fall", run)) %>%
  distinct()

# Check....Must all be Fall...or something wrong in DB
...
all_surveys = all_surveys %>%
  select(WRIA, StreamName, LLID, Type, Date, StatWeek,
         ...
         run, SurveyIntent, SurveyStatus, LM, LF, LSND, LJ, DM,
...
# Verify run_year are all NA for zero counts
...
# Write to xlsx
out_name = paste0("data_query/FallChum_WRIA_22.xlsx")
